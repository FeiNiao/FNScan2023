...
filename = "./reps/中远麒麟堡垒机admin_SQL注入-"+str(time.strftime("%Y-%m-%d", time.localtime()))
headers = {
...
def zyadminSQL(url):
    try:
        ...
        if el >= 5:
            print(buter.colors.GREEN+  f"存在中远麒麟堡垒机admin_SQL注入 url : {url}" + buter.colors.END)
            reptext = reps.url
            wirtefile.writefiles(filename,reptext)
        else:
            print(buter.colors.RED+ f"未发现中远麒麟堡垒机admin_SQL注入 url : {url} " + buter.colors.END)
